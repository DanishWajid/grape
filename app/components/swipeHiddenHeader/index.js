...
class SwipeHiddenHeader extends Component {
  ...
  calcHeaderOffsetY = (currentOffsetY, lastOffsetY) => {
    const startHiddenOffset =
      this.props.startHiddenHeaderOffset !== undefined
        ? this.props.startHiddenHeaderOffset
  ...
  calcScrollViewTop = () => {
    if (
      ...
      this.state.offsetY < this.state.headerHeight
    ) {
  ...
  onScroll = e => {
    ...
    if (!this.props.renderScrollComponent) {
      this.props.scrollViewProps.onScroll &&
        this.props.scrollViewProps.onScroll(e);
    }
